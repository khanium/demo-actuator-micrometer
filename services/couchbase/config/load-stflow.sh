...
# Read the JSON file
home_dir="/Users/josemolinagonzalez/couchbaseBin"
    ...
    if [ -d "$folder_path" ]; then
        ...
        if [ ${#json_files[@]} -gt 0 ]; then
            ...
            for json_file in "${json_files[@]}"; do
              ...
              $home_dir/cbimport json --format list -c 'localhost' -u Administrator -p "password" -d "file://$json_file" -b "$bucket_name" --scope-collection-exp "_default._default" -g "$key_exp"
            done
        else
